resource "aws_api_gateway_integration" "search_lambda_integration" {
  ...
  resource_id             = aws_api_gateway_resource.search_proxy_path.id
...
resource "aws_api_gateway_resource" "search_proxy_path" {
  parent_id   = aws_api_gateway_resource.search_path.id
  path_part   = "{proxy+}"
  rest_api_id = aws_api_gateway_rest_api.dos_comparison_tool_api_gateway.id
  depends_on = [
